...
from .morphomnist.morphomnist.morpho import ImageMoments, ImageMorphology, bounding_parallelogram
from .morphomnist.morphomnist.perturb import Deformation, Perturbation


def _get_disk(radius: int, scale: int):
    mag_radius = scale * radius
    mag_disk = morphology.disk(mag_radius, dtype=np.float64)
    disk = transform.pyramid_reduce(mag_disk, downscale=scale, order=1)
    return disk
